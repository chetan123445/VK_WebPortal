...
export default function ProfileMenu({ userEmail, userData, avatarStyle, onProfileUpdate, forceOpen }) {
  ...
  useEffect(() => {
    if (!getToken()) {
      if (typeof window !== 'undefined') {
        ...
        // Remove all UI and force a hard redirect
  ...
  // UI rendering for modal/profile form
  if (forceOpen || open) {
    return (
      <div style={{
        position: forceOpen ? 'relative' : 'fixed',
        top: forceOpen ? undefined : 0,
        left: forceOpen ? undefined : 0,
        width: '100vw',
        minHeight: '100vh',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        background: forceOpen ? 'rgba(0,0,0,0.03)' : 'rgba(0,0,0,0.5)',
        zIndex: 3000,
        backdropFilter: forceOpen ? undefined : 'blur(4px)'
      }}>
        <div style={{
          background: '#fff',
          color: '#222',
